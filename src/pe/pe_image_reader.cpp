...
namespace stereo {
    namespace pe {
        ...
        {
            auto file = fileutil::load_entire_file(file_path);
        ...
        ReadResult PEImageReader::read_metadata()
        {
            ...
            auto signature = ptrutil::read32(&metadata_ptr);

            if (signature == 0x424A5342) {

                image_->md_version_major = ptrutil::read16(&metadata_ptr);

                image_->md_version_minor = ptrutil::read16(&metadata_ptr);

                // Skip the 4 reserved bytes
                metadata_ptr += 4;

                auto version_string_len = ptrutil::read32(&metadata_ptr);

                char buffer[255];
                strncpy(buffer, (char*)metadata_ptr, version_string_len);
                image_->md_version_string = strutil::utf8str_to_utf16wstr(std::string(buffer, version_string_len));
        ...
        bool PEImageReader::read_streams(ReadResult read_result)
        {
            ...
            auto num_streams = ptrutil::read16(&stream_ptr);

            for (auto i = 0; i < num_streams; i++)
            {
                if (strncmp((char*)stream_ptr + 8, "#~", 3) == 0)
                {
                    image_->heap_tables.data = base_metadata_ptr + ptrutil::peek32(stream_ptr);
                    image_->heap_tables.size = ptrutil::peek32(stream_ptr + 4);
                    stream_ptr += 8 + 3;
                }
                else if (strncmp((char*)stream_ptr + 8, "#Strings", 9) == 0)
                {
                    image_->heap_strings.data = base_metadata_ptr + ptrutil::peek32(stream_ptr);
                    image_->heap_strings.size = ptrutil::peek32(stream_ptr + 4);
                    stream_ptr += 8 + 9;
                }
                else if (strncmp((char*)stream_ptr + 8, "#US", 4) == 0)
                {
                    image_->heap_us.data = base_metadata_ptr + ptrutil::peek32(stream_ptr);
                    image_->heap_us.size = ptrutil::peek32(stream_ptr + 4);
                    stream_ptr += 8 + 4;
                }
                else if (strncmp((char*)stream_ptr + 8, "#Blob", 6) == 0)
                {
                    image_->heap_blob.data = base_metadata_ptr + ptrutil::peek32(stream_ptr);
                    image_->heap_blob.size = ptrutil::peek32(stream_ptr + 4);
                    stream_ptr += 8 + 6;
                }
                else if (strncmp((char*)stream_ptr + 8, "#GUID", 6) == 0)
                {
                    image_->heap_guid.data = base_metadata_ptr + ptrutil::peek32(stream_ptr);
                    image_->heap_guid.size = ptrutil::peek32(stream_ptr + 4);
                    stream_ptr += 8 + 6;
                }
                else if (strncmp((char*)stream_ptr + 8, "#Pdb", 5) == 0)
                {
                    image_->heap_pdb.data = base_metadata_ptr + ptrutil::peek32(stream_ptr);
                    image_->heap_pdb.size = ptrutil::peek32(stream_ptr + 4);
                    stream_ptr += 8 + 5;
                }
                else
                {
        ...
        bool PEImageReader::read_tables()
        {
            ...
            for (auto table = 0; table < 64; table++)
            {
                ...
                {
                    image_->tables[table].rows = ptrutil::peek32(reinterpret_cast<u8*>(rows));
                }
